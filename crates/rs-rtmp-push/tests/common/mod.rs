...
///
/// This version therefore mirrors xiu's real `ServerSession` byte-for-byte
/// (`rtmp-0.6.5/src/session/server_session.rs`) using the same server-side
/// writers, and substitutes ONLY the publish onStatus code. It uses the complex
/// `HandshakeServer` (which auto-falls back to simple for the pusher's
/// `SimpleHandshakeClient`, exactly as the real server does) and feeds its
/// `get_remaining_bytes()` into the unpacketizer.
///
    ...
    loop {
        ...
        loop {
            match unpacketizer.read_chunks() {
                ...
                // NotEnoughBytes / any partial-read state -> go read more.
                Ok(_) => break,
                Err(_) => break,
            }
        }
