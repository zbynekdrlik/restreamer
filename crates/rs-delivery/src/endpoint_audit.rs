...
/// One row in the endpoint restart audit history. #212 removed the
/// ffmpeg-subprocess push path, so nothing writes this record any more — it is
/// retained as an inert serialized dashboard type (the `restart_history` wire
/// field always renders empty) rather than churn the VPS→host→UI wire format
/// and its e2e assertions in this PR.
...
/// Audit record emitted on every reconnect of a Rust-pusher endpoint.
/// Mirrors `FfmpegRestartRecord` so the dashboard can render either source
/// uniformly. See spec sec 5.5.
