//! C4 (#239) — REAL runtime behavioral rescue tests: Groups B + C.
//!
...
//!
//! Group C (#237 / #251) drives a real producer PANIC through the real
//! `endpoint_loop` so the respawn arm + `on_producer_finished` resume math
//! are covered behaviourally — the DrainingFetcher tests in Group B never
//! reach that arm (their producer never FINISHES, it just drains).
//!
