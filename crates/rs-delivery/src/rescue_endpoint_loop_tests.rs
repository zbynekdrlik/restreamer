//! C4 (#239) — REAL runtime behavioral rescue tests: Group B (end-to-end
//! drain activates rescue). Group C (producer-respawn tests that injected a
//! mock ffmpeg `OutputProcess`) was removed in #212 with the ffmpeg push path.
//!
//! Split from `rescue_behavioral_tests.rs` (which keeps Group A — the
//! shared rescue push loop) to stay under the project's 1000-line-per-file
//! cap. See that file's module doc for the shared background on why these
//! tests exist (replacing the R1/R2/R3 source-grep tests that asserted
//! source SHAPE instead of BEHAVIOUR).
//!
//! Group B drives the REAL `endpoint_loop` (producer + consumer pipeline)
//! with a draining `ChunkFetcher` and asserts that, for BOTH a fast and a
//! non-fast endpoint, a sustained drain ACTIVATES rescue: the
...
//! here — the byte-level "rescue clip actually pushed" assertion lives in
//! `rescue_behavioral_tests.rs`'s Group A. What IS observable end-to-end is
//! the ACTIVATION: the `RescueActivated` audit row + `delivery_mode ==
//! "rescue"`. We assert exactly that.
//!
...
fn ep_cfg(alias: &str, is_fast: bool) -> EndpointConfig {
    EndpointConfig {
...
async fn nonfast_clean404_drain_activates_rescue() {
    ...
    let cfg = ep_cfg("nonfast-404", false);
    let fetcher = DrainingFetcher {
        available_up_to: 3,
        duration_ms: 2000,
        err_drain: false,
    };
    let (ring, stats) = run_until_rescue_or_timeout(cfg, fetcher).await;
...
async fn nonfast_err_drain_activates_rescue() {
    ...
    let cfg = ep_cfg("nonfast-err", false);
    let fetcher = DrainingFetcher {
        available_up_to: 3,
        duration_ms: 2000,
        err_drain: true,
...
async fn fast_err_drain_escalates_to_rescue() {
    ...
    let cfg = ep_cfg("fast-rust-err", true);
    let fetcher = DrainingFetcher {
        available_up_to: 3,
        duration_ms: 2000,
        err_drain: true,
    };
    let (ring, _stats) = run_until_rescue_or_timeout(cfg, fetcher).await;
