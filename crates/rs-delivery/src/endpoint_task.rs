/// Per-endpoint delivery task: S3 poll -> disk cache -> in-process Rust RTMP
/// pusher (`rs_rtmp_push`). Producer -> bounded channel (~20s) -> Consumer.
...
// every existing `crate::endpoint_task::{ChunkFetcher, PrefetchedChunk}` import
// path keeps resolving unchanged.
#[path = "endpoint_traits.rs"]
mod endpoint_traits;
pub use endpoint_traits::ChunkFetcher;
pub(crate) use endpoint_traits::PrefetchedChunk;

// `FfmpegRestartRecord` / `RESTART_HISTORY_CAP` / `RtmpPushAuditRecord` are
// re-exported here so `api.rs` and the endpoint-stats wire struct keep
// resolving them at the `endpoint_task` level. `FfmpegRestartRecord` is now an
// inert serialized dashboard type (nothing writes it after #212 removed the
// ffmpeg push path); `RESTART_HISTORY_CAP` still caps the rust-pusher history.
pub use crate::endpoint_audit::{FfmpegRestartRecord, RESTART_HISTORY_CAP, RtmpPushAuditRecord};

#[path = "endpoint_consumer_helpers.rs"]
mod consumer_helpers;
use crate::disk_cache_push_sample::{PushSampleCtx, emit_push_sample};
use consumer_helpers::{RustPushAction, handle_rust_push, maybe_refill_throttle};
...
/// Consumer task: pulls pre-fetched chunks from the channel and writes each
/// self-contained FLV chunk to the in-process Rust RTMP pusher
/// (`rs_rtmp_push`). Never makes S3 calls -- zero network I/O.
///
/// Pacing is done by the producer's disk-cache delay + the pusher's own
/// monotonic-timestamp logic; the consumer just forwards complete FLV chunks
/// as they arrive.
...
async fn consumer_task(
    ...
    // The Rust-path output handle. `rs_rtmp_push` is the only push backend
    // (#212 removed the ffmpeg-subprocess path); lazy-connects on first write.
    // Constructed below once the RTMP URL is built; rescue can drop + rebuild
    // it, so it stays a `mut Option`.
    let mut rust_pusher: Option<RtmpPusher>;
    ...
    loop {
        ...
        // Periodic heartbeat. The Rust pusher lazy-reconnects on the next
        // push after an error, so there is no process lifecycle to manage
        // here — push errors are handled in the write section below.
        ...
        //
        // FAST endpoints: a short producer gap triggers the never-crash
        // keepalive (freeze last chunk → default rescue) on the SAME rtmp
        // session, so starvation never tears the connection down. Non-fast
        // endpoints keep the existing 8s `run_outage_rescue` select! arm.
        let chunk = if ep_cfg.is_fast {
            tokio::select! {
                maybe_chunk = rx.recv() => {
                    match maybe_chunk {
                        Some(c) => {
        ...
        // Rust RTMP pusher write path (the only backend after #212). Each S3
        // chunk is a self-contained FLV with its own 9-byte header; the rust
        // pusher's push_flv_bytes parses it as a complete FLV and applies its
        // own monotonic-timestamp logic via state.last_output_ts_ms.
...
/// Core endpoint loop -- generic over ChunkFetcher for testability.
/// Orchestrates buffer fill, then spawns producer-consumer pipeline.
#[allow(clippy::too_many_arguments)]
pub async fn endpoint_loop<F: ChunkFetcher + 'static>(
    fetcher: F,
    ep_cfg: EndpointConfig,
    start_chunk_id: i64,
    delivery_delay_ms: u64,
