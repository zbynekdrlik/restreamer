...
///
/// Used by `DeliveryOrchestrator::on_vps_ready` to drive the fast-endpoint
/// live-edge recompute at VPS-ready time. Extracted as a free fn so it can
/// be unit-tested without the orchestrator's full HTTP machinery.
...
impl DeliveryOrchestrator {
    ...
    pub async fn poll_and_init(
        ...
        // VPS is delivering. Without this, fast endpoints would stream from a
        // chunk_id computed 30-50s ago (before VPS creation finished), staying
        // behind live edge indefinitely. See spec
        // docs/superpowers/specs/2026-05-11-cache-metric-and-start-reset-design.md.
        let on_ready_client = reqwest::Client::new();
        if let Err(e) = self
            .on_vps_ready(event_id, &instance, start_chunk_id, &on_ready_client)
            .await
        {
    ...
    pub async fn on_vps_ready(
        ...
        let updates = compute_fast_endpoint_updates(&self.pool, event_id).await?;
        if updates.is_empty() {
            return Ok(());
        }
