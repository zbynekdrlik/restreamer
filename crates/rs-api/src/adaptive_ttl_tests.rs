...
fn good_and_no_issue_returns_60s() {
    assert_eq!(
        ttl_for_health(&health("good", None, None)),
...
fn bad_returns_15s() {
    assert_eq!(
        ttl_for_health(&health("bad", Some("videoIngestionStarved"), None)),
...
fn ok_returns_15s() {
    assert_eq!(
        ttl_for_health(&health("ok", Some("gopSizeLong"), None)),
        Duration::from_secs(15)
    );
}
...
fn good_with_top_issue_returns_15s() {
    ...
    assert_eq!(
        ttl_for_health(&health("good", Some("framerateHigh"), None)),
        Duration::from_secs(15)
    );
}
...
fn error_path_returns_15s() {
    assert_eq!(
        ttl_for_health(&health("unknown", None, Some("probe_error"))),
        Duration::from_secs(15)
    );
}
...
fn quota_throttled_returns_15s() {
    ...
    assert_eq!(
        ttl_for_health(&health("unknown", None, Some("quota_throttled"))),
        Duration::from_secs(15)
    );
}
