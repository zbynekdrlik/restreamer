...
class DeliveringManger:
    ...
    def init_delivery(self):
        self.session.get(f"{self.get_url}/connect", params={"init_data": self.stream_data})
    ...
    def send_init_data(self, chunk_id=None, endpoint_id=None):
        if endpoint_id is None:
            endpoints = self.streaming_event.end_points.exclude(is_fast=True).values(
                "alias", "service_type", "stream_key"
            )
        ...
        data = {"endpoints": list(endpoints), "chunk_id": chunk_id, "stream_id": stream_id}
