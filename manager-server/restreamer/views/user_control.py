...
from restreamer.scheduler import cancel_delete_instance_jobs, delete_instance_schedule, schedule_init_stream
from restreamer.tasks import end_stream, init_fast_stream, init_stream
    ...
    def post(self, request, *args, **kwargs):
        ...
        if streaming_event.delivering_activated:
            try:
                ...
                messages.error(request, "Error initialized stream!")

            messages.success(request, "Endpoint added stream initialized!")
...
class StreamSchedulerView(View):
    ...
    def post(self, request):
        ...
        if (
            data.get("start_time")
            and data.get("chunk_time")
            and len(data.get("end_points", 0)) >= 1
            and data.get("streaming_event")
        ):
...
def user_history(request, user_id):
    ...
    for event in streaming_events:
        for record in event.history.all():
            ...
            if prev_record:
                ...
                changes = {
                    field: (getattr(prev_record, field), getattr(record, field)) for field in diff.changed_fields
                }
            streaming_event_history.append({"record": record, "changes": changes})
    ...
    for endpoint in endpoints:
        for record in endpoint.history.all():
            ...
            endpoints_history.append({"record": record, "changes": changes})
    ...
    for record in user.history.all():
        ...
        users_history.append({"record": record, "changes": changes})
