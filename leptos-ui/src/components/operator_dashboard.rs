                            ...
                            >
                                ...
                                {move || {
                                    ...
                                    let (cache_secs, target_label, progress, bar_class) = if ep.is_fast {
                                        ...
                                        // Non-fast: original behaviour. During prefill (chunks_processed=0)
                                        // fall back to the global ps.cache_duration_secs which the
                                        // backend caps at 1.5x target (#187). Per-service threshold
                                        // multiplier from utils.rs.
