...
#   irm https://raw.githubusercontent.com/zbynekdrlik/restreamer/dev/local-client-rs/setup-deploy.ps1 | iex
...
if (-not $isAdmin) {
    ...
    Start-Process powershell -ArgumentList "-NoProfile -ExecutionPolicy Bypass -Command `"irm https://raw.githubusercontent.com/zbynekdrlik/restreamer/dev/local-client-rs/setup-deploy.ps1 | iex`"" -Verb RunAs
    exit
}
