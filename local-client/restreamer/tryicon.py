...
class TrayIcon:
    ...
    def monitor_endpoint_redis_queue(self, icon):
        while True:
            ...
            if status != "running":
                menu_items.append(
                    pystray.MenuItem(text="Start", action=lambda: self.tray_icon_actions("start_endpoint"))
                )
            else:
                menu_items.append(
                    pystray.MenuItem(text="Restart", action=lambda: self.tray_icon_actions("restart_endpoint"))
                )

            menu_items.append(pystray.MenuItem(text="Log File", action=lambda: self.open_log_file("endpoint")))
    ...
    def monitor_inpoint_redis_queue(self, icon):
        ...
        while True:
            ...
            menu_items.append(pystray.MenuItem(text="Delete Chunks", action=lambda: delete_local_chunks()))

            if status != "running":
                menu_items.append(
                    pystray.MenuItem(text="Start", action=lambda: self.tray_icon_actions("start_inpoint"))
                )
            else:
                menu_items.append(
                    pystray.MenuItem(text="Restart", action=lambda: self.tray_icon_actions("restart_inpoint"))
                )

            # Always add the "Log File" option for inpoint
            menu_items.append(pystray.MenuItem(text="Log File", action=lambda: self.open_log_file("inpoint")))
